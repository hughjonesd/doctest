...
1. Alter your package DESCRIPTION to add the `doctest` roclet to roxygen:
  ```
...
or `devtools::document()` to create documentation. You will see new files 
labelled `test-doctest-<topic>.R` in the `tests/testthat` directory. You should
also see Rd files created as normal in the `man/` directory, including `\examples` sections.

4. Add `@expect` tags to your `@doctest` sections.

5. Run `roxygenize()` again. Your tests will be recreated with new expectations.
...
By default, the doctest uses the whole example. `@pause` stops including the
