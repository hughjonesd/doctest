...
The next expression will be substituted as the first argument
...
example. `@doctest <test-name>` changes to a new test.
...
`@expect` replaced by `# expect` (or `@expectRaw` replaced by `# expectRaw`).
Comments must be on their own line, and will remain in the example code. 
...
*substitute* for them. For example, testing difficult corner cases
would not usually be appropriate in documentation. Use doctest for relatively simple tests of basic functionality, or to catch when user-visible output
changes using `expect_snapshot()`:
