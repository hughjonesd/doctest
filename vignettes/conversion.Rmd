...
a few deleted empty lines in the examples. I thought these were unimportant, so 
I was now ready to make my first commit.

At this stage, you might want to create a new branch with your commits, using
`git branch` on the command line, or clicking the "new branch" button in RStudio's
git pane. I was gung ho, so I just put 
[my commit](https://github.com/hughjonesd/onetime/commit/1c6400cac3528c27ffd2d397250da527fa2dbf7d?diff=split) 
on the master branch.

I actually made one exception: I left the `@examples` tag unchanged in the
...
a doctest would not be the right approach here, so I left it as is.


# Adding doctests

So far, nothing has actually changed. To generate doctests from my examples,
I needed to add some testthat expectations to my code. 

If you know testthat, doctest expectations will look very familiar. In testthat,
you write:

```r
expect_equal(x, 2.718282)
```

In a `@doctest` roxygen section, this becomes:

```r
#' @expect equal(2.718282)
#' x
```

where `x` is part of your example. Similarly, where in testthat you might write:

```r
expect_warning(mean("foo"), "not numeric")
```

in a `@doctest` section this would be:
...
* You use the `@expect` tag to create an expectation;
* After `@expect`, you write a testthat expectation, minus the `expect_` prefix 
  to the function call;
* The next R object after the `@expect` line becomes the first argument to the
  expectation.
...
again. Lo, it produced a new file under `tests/testthat`, called
...
Notice that the test file has a warning at the top. If you edit this file,
...
removing expectations or even replacing `@doctest` back with `@examples`.

My next doctest was more complex. The code looked like this:
...
I decided to just test the first call to `onetime_dir()`. The second call
would return a temporary directory, which would be different between different
R sessions, so I wasn't sure how to test it usefully, and I wanted to
keep things simple. In fact, to skip unnecessary code from the test, I used
the `@omit` tag:
...
`@omit` omits everything after it from the generated test, so this created
a very simple test file in `test-doctest-onetime_dir`:
...
Lastly, I added tests for my final functions. There's nothing new to see here,
so I'll just let you examine the [commit](https://github.com/hughjonesd/onetime/commit/d3be66faabee09929deceafc9598488efd0ae8ee?diff=split).
