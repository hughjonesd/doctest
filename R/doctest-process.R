...
process_expectation <- function (expectation, test) {
  code <- expectation$expect

  further_lines <- expectation$line +
                   seq_len(length(test$lines) - expectation$line)
  lines_expression <- parse(text = test$lines[further_lines])
  target <- lines_expression[[1]] # a call

  code <- trimws(code)
  code <- paste0("expect_", code)
  code <- parse(text = code, n = 1)[[1]]

  ast_has_a_dot <- function (x) {
    if (length(x) == 1) return(typeof(x) == "symbol" && as.character(x) == ".")
    if (length(x) > 1) any(unlist(lapply(x, ast_has_a_dot)))
  }
  has_a_dot <- ast_has_a_dot(code)

  code <- if (has_a_dot) {
                   do.call("substitute", list(code, list(. = target)))
                 } else {
                   as.call(c(
                            code[[1]],
                            target,
  ...
  test$lines <- append(test$lines, code, after = expectation$line)
